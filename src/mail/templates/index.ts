export { applicationConfirmationTemplate } from './application-confirmation.template';
export type { ApplicationConfirmationData } from './application-confirmation.template';
export { applicationStatusUpdateTemplate } from './application-status-update.template';
export type { ApplicationStatusUpdateData } from './application-status-update.template';
export { baseTemplate } from './base.template';
export type { BaseEmailOptions } from './base.template';
export { beauticianDispatchSuspensionTemplate } from './beautician-dispatch-suspension.template';
export type {
  DispatchSuspensionEmailData,
  DispatchSuspensionEmailKind
} from './beautician-dispatch-suspension.template';
export {
  serviceCompletedTemplate
} from './beautician-home-service-lifecycle.template';
export { beauticianKycResultTemplate } from './beautician-kyc-result.template';
export { beauticianProfileReviewTemplate } from './beautician-profile-review.template';
export { bookingConfirmationTemplate } from './booking-confirmation.template';
export type { BookingConfirmationData } from './booking-confirmation.template';
export { contactFormSubmissionTemplate } from './contact-form-submission.template';
export type { ContactFormSubmissionData } from './contact-form-submission.template';
export { depositSuccessTemplate } from './deposit-success.template';
export type { DepositSuccessData } from './deposit-success.template';
export { guestBookingNotificationTemplate } from './guest-booking-notification.template';
export type { GuestBookingNotificationData } from './guest-booking-notification.template';
export { interviewScheduledTemplate } from './interview-scheduled.template';
export type { InterviewScheduledData } from './interview-scheduled.template';
export { lowStockAlertTemplate } from './low-stock-alert.template';
export type { LowStockAlertData } from './low-stock-alert.template';
export { offerDeclinedTemplate } from './offer-declined.template';
export type { OfferDeclinedData } from './offer-declined.template';
export { offerExtendedTemplate } from './offer-extended.template';
export type { OfferExtendedData } from './offer-extended.template';
export { otpTemplate } from './otp.template';
export { referralRewardTemplate } from './referral-reward.template';
export type { ReferralRewardData } from './referral-reward.template';
export { resetPasswordTemplate } from './reset-password.template';
export { shopOrderConfirmationTemplate } from './shop-order-confirmation.template';
export type { ShopOrderConfirmationData } from './shop-order-confirmation.template';
export { staffBirthdayTemplate } from './staff-birthday.template';
